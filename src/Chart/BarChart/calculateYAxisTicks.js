...
import styles from './../common/drawGoalLabels.css';

export default (goals, nextGoal, yourScore, highestYValue, goal, hasStarted, yScale, gnomeIcon) => {
  ...
  if (goal) {
    ...
    return nonClustered.map(ms => {
          ...
          const label = ms;
          const path = (ms === goal) ? paths.trophy : ((ms <= yourScore) ? paths.check : [gnomeIcon ? paths.gnome : paths.leaf]);
